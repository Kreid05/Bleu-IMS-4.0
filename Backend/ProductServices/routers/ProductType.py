...
async def create_product_type(
    ...
    try:
        ...
        if await cursor.fetchone():
            raise HTTPException(status_code=400, detail="Product type name already exists in primary service (8001)")
        ...
        id_row = await cursor.fetchone() # output clause make fetchone to return id
        print(f"DEBUG: id_row from OUTPUT clause: {id_row}") # for debugging
        
        if id_row and id_row[0] is not None:
            new_product_type_id = int(id_row[0])
        else:
            # this path means INSERT might have happened but OUTPUT didn't return the ID,
            # or returned NULL. 
            await conn.rollback() 
            print("ERROR: id_row from OUTPUT clause was None, or id_row[0] was None.")
            raise HTTPException(status_code=500, detail="Failed to retrieve ID after insert (using OUTPUT clause). Please check table 'ProductType' definition: 'productTypeID' must be an IDENTITY column.")

        await conn.commit() # commit only if INSERT and ID retrieval were successful
        local_save_success = True
        print(f"Product type '{request.productTypeName}' (ID: {new_product_type_id}, SizeRequired: {request.SizeRequired}) created successfully in primary service (8001)")

    except HTTPException:
        raise
    except Exception as e:
        print(f"General error saving to primary service (8001): {e}")
        try:
            await conn.rollback()
        except Exception as rb_exc:
            print(f"Rollback failed during general exception: {rb_exc}")
        raise HTTPException(status_code=500, detail=f"Failed to save to primary DB: {str(e)}")
    finally:
        if cursor:
            await cursor.close()
        if conn:
    ...
    # if local save successful - send to POS
    sync_message = ""
    if local_save_success and new_product_type_id is not None:
        SECONDARY_SERVICE_URL = "http://localhost:9001/create" # pos endpoint
        async with httpx.AsyncClient() as client_to_9001:
            try:
                ...
                print(f"Attempting to sync create to POS (port 9001): {payload_to_9001}")
                response_from_9001 = await client_to_9001.post(
                    SECONDARY_SERVICE_URL,
                    json=payload_to_9001,
                    headers=headers_to_9001
                )

                if response_from_9001.status_code in [200, 201]:
                    sync_message = f"Product type also synced to POS (9001). POS Response: {response_from_9001.json()}"
                    print(f"Successfully synced to POS (9001): {response_from_9001.json()}")
                else:
                    sync_message = f"Failed to sync to POS (9001). Status: {response_from_9001.status_code}, Info: {response_from_9001.text}"
                    print(sync_message)

            except httpx.HTTPStatusError as e_sync:
                sync_message = f"HTTP error syncing to POS (9001): {e_sync.response.status_code} - {e_sync.response.text}"
                print(sync_message)
            except httpx.RequestError as e_req:
                sync_message = f"Request error syncing to POS (9001): {str(e_req)}"
                print(sync_message)
            except Exception as e_gen:
                sync_message = f"An unexpected error occurred during sync to POS (9001): {str(e_gen)}"
                print(sync_message)
    
    return {
        "message": f"Product type created successfully in primary service (8001). {sync_message}".strip(),
